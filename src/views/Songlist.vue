    ...
    <div class="albumns">
      ...
      <div class="songlist">
        <div class="count_box">
          ...
          <a
            class="collect"
            @click="likesonglist($store.state.songlistid)"
            data-type="2"
            href="javascript:;"
          >
            收藏
...
export default {
  mixins: [myMinxin],
  created() {
    console.log("sa", this.$store.state.songlistid);
    this.$axios
      .get(
        `https://api.bzqll.com/music/tencent/songList?key=579621905&id=${
          this.$store.state.songlistid
        }`
      )
      .then(res => {
        res = res.data;
        this.albumarr = res.data.songs;
        this.albumname = res.data.desc;
        this.singer = res.data.author;
  ...
  methods: {
    ...
    likesonglist(val) {
      this.$store.commit("likesonglistarr", val);
    }
  },
  watch: {}
};
